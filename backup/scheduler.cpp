...
int main(int argc, char **argv) {
	try {
		...
		string bdefault("selected=\"selected\">");
    	for (int i=0; i < 12; i++) {
    		string tempr=ss.str();
    		stringstream number;
    		number << i;
    		//label a;; the fields sequentially
    		boost::replace_all(tempr, "XXX", number.str());

    		//autofill the defaults for 12-well format
    		if (i >3 && i <8) boost::replace_all(tempr, "YYY>B", bdefault + string("B"));
    		if (i >7) boost::replace_all(tempr, "YYY>C", bdefault + string("C"));
    		if (i >7) boost::replace_all(tempr, "YYY>C", bdefault + string("C"));
    		int nrow = i % 4;
    		if (nrow==0) boost::replace_all(tempr, "ZZZ>1", bdefault + string("1"));
    		if (nrow==1) boost::replace_all(tempr, "ZZZ>2", bdefault + string("2"));
    		if (nrow==2) boost::replace_all(tempr, "ZZZ>3", bdefault + string("3"));
    		if (nrow==3) boost::replace_all(tempr, "ZZZ>4", bdefault + string("4"));
